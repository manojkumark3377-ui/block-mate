...
`;
    ...
    if (content.match(/const navigate = useNavigate\(\);/)) {
        ...
        // Fallback for CategoryList where it might not match well
    ...
    // Regular expression to match a button element containing <FaTrash />
    const faTrashRegex = /(<button[^>]*onClick=\{.*?handleDelete.*?\}[^>]*>[\s\S]*?<FaTrash \/>[\s]?<\/button>)/g;
    ...
    const textDeleteRegex = /(<button[^>]*onClick=\{.*?handleDelete.*?\}[^>]*>\s*Delete\s*<\/button>)/g;
